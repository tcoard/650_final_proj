...
EMB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "embeds", f"protbert_embeddings{DATASET}.npy")
SEQ_SUBSET = 100
BATCH_SIZE = 10
NUM_FOLDS = 5
MAX_SEQ_LEN = 1024
...
def make_trial_data():
    ...
    sequences = np.load(open(EMB_PATH, "rb"))
    resistances = np.load(open(f"{EMB_PATH}_lookup", "rb"))
    ...
    #_, df, _ = np.split(df.sample(frac=1, random_state=42), [int(0.95 * len(df)), int(0.975 * len(df))])

    #_, df, _ = np.split(df.sample(frac=1, random_state=42), [int(0.00 * len(df)), int(0.50 * len(df))])
    # print(df.shape)
    print(resistance_categories)
    ...
    for header, emb in header_emb.items():
        drug = header.split("|")[-1].strip()
        if not drug_emb.get(drug):
            drug_emb[drug] = [emb]
        else:
            drug_emb[drug].append(emb)
...
def pad_text(tokenized_text):
    return torch.tensor(
        ...
        [list(el) for el in tokenized_text], dtype=torch.float32
    ).to(DEVICE)
...
def create_embeddings():
    ...
    # get total seq_count
    # make np.array with all zeros
    # add embedding to array
    # make dictioary lookup table {"drug_class": [1,30,55]}
    ...
    max_emb_len = 1024
    res_lookup = np.empty(tot_seq_num, dtype="U25")
    final_emb = np.empty((tot_seq_num, max_emb_len, MAX_SEQ_LEN), dtype=float)
    with open(FASTA_PATH, "r") as fasta:
        res = ""
        seq_index = 0
        for count, line in enumerate(fasta):
            ...
            if line.startswith(">"):
                res = line.strip().split("|")[-1]
            elif res_count[res] >= MIN_DRUG_RES and len(line) <= MAX_SEQ_LEN:
                embedding = fe(" ".join(line))
                seq_emb = embedding[0][1 : len(line) + 1]
                if len(seq_emb[0]) != max_emb_len:
                    print(line)
                    raise TypeError

                for i, emb in enumerate(seq_emb):
                    for ii, val in enumerate(emb):
                        final_emb[seq_index][i][ii] = val
                res_lookup[seq_index] = res
                seq_index += 1

    np.save(open(EMB_PATH, "wb"), final_emb)
    np.save(open(f"{EMB_PATH}_lookup", "wb"), res_lookup)

    return final_emb, res_lookup
